#include <stdio.h>
#include <string.h>
#include <inttypes.h>                  // รองรับ PRId32 specifier ใน ESP-IDF v6.0
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT) {
        ...
        default:
            ESP_LOGI(TAG, "[EVENT FORENSIC]: WIFI_EVENT ID %" PRId32 " received", event_id);
            break;
        }
...
// Function to test Wi-Fi connection with specific credentials
static void test_wifi_connection(const char *test_title, const char *ssid,
    ...
    // Clear event bits before starting test
    xEventGroupClearBits(s_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT);

    // Zero-initialize struct (ESP-IDF v6.0 compliance to avoid uninitialized memory warning)
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "==================================================================");
    ESP_LOGI(TAG, "  Lab 5.2: Wi-Fi Connection & IP Assignment (ESP-IDF v6.0 Ready)");
    ESP_LOGI(TAG, "==================================================================");

    // 5.2.1 Connecting with Correct SSID & Password
    test_wifi_connection("Experiment 5.2.1: Connection Test - Correct Credentials",
                         EXAMPLE_ESP_WIFI_SSID, EXAMPLE_ESP_WIFI_PASS);

    vTaskDelay(pdMS_TO_TICKS(2000));

    // 5.2.2 Connecting with Wrong SSID (No AP Found)
    test_wifi_connection("Experiment 5.2.2: Connection Test - Wrong SSID (No AP Found)",
                         "NON_EXISTENT_SSID_9999", "12345678");

    vTaskDelay(pdMS_TO_TICKS(2000));

    // 5.2.3 Connecting with Correct SSID but Wrong Password
    test_wifi_connection("Experiment 5.2.3: Connection Test - Wrong Password (Auth Fail)",
