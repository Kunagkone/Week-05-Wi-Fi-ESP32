...
| :---: | :--- | :---: | :---: | :---: |
| **5.1.1** | สแกนทั่วไปทุก Channel | Scan success | 14 network(s) Found | 2498 ms |
| **5.1.2** | กำหนดสแกนเฉพาะ Channel 1 | Scan success | 6 network(s) Found | 200 ms |
| **5.1.3** | กำหนดสแกน SSID ที่มีจริง | Scan success | 3 network(s) Found | 2499 ms |
...
| :---: | :--- | :--- | :---: | :---: | :--- |
| 1 | KMITL-WIFI| 78:17:BE:C0: 7D:A1 | -49 dBm | 1  |  OPEN (No Password) |
| 2 |KMITL-Legacy | 78:17:BE:C0:7D:A0| -49 dBm | 1| WPA2 ENTERPRISE|
| 3 | KMITL-IOT| 78:17:BE:C0: 7D:A2 | -49 dBm | 1 | WPA2 PSK |
| 4 |S24 Ultra Koson | 7A:6D:06:A9:DE: 5A | -49 dBm | 6|  WPA2 PSK|
| 5 |KBBK-IPHONE | 2E:EC:B9: E4:42:C3 |  -49 dBm |6 | WPA2 WPA3_PSK|
![alt text](image.png)
---
...
การลดเวลา: การกำหนด .channel = 1 ทำให้โมดูลวิทยุสแกนและรอฟัง Beacon/Probe Response เฉพาะในช่องความถี่ที่ระบุเท่านั้น ไม่ต้องสแกนไล่ทีละช่องจนครบทั้ง 13 ช่องความถี่ จึงลดเวลาการสแกนลงจากประมาณ 2498 ms (สแกนทุกช่อง) เหลือเพียงประมาณ 200 ms  ข้อจำกัด: อุปกรณ์จะไม่สามารถตรวจพบ Access Point (AP) ที่ทำงานอยู่ในช่องความถี่อื่น (Channel 2–13) ได้ หาก AP เป้าหมายมีการเปลี่ยนช่องสัญญาณแบบอัตโนมัติ (Auto Channel) หรือตั้งค่าช่องสัญญาณไว้ตรงกับช่องอื่น การสแกนเจาะจงเฉพาะช่องจะไม่พบเครือข่ายนั้น
...
esp_wifi_scan_start(): ส่งคืนค่า ESP_OK (0x0) เนื่องจากกระบวนการสแกนทางฮาร์ดแวร์ทำงานสำเร็จสมบูรณ์โดยไม่เกิดข้อผิดพลาดของไดรเวอร์  esp_wifi_scan_get_ap_num(): ส่งคืนค่า ESP_OK (0x0) โดยตัวแปรนับจำนวน ap_count ถูกกำหนดค่าเป็น 0 (พบ 0 เครือข่าย)  esp_wifi_scan_get_ap_records(): หากโปรแกรมตรวจพบ ap_count == 0 จะข้ามการเรียกใช้งาน แต่หากเรียกใช้ ฟังก์ชันจะส่งคืน ESP_OK พร้อมคืนค่าโครงสร้างข้อมูล 0 รายการ  
...
ค่า -45 dBm แสดงถึงสัญญาณที่มีความแรงและความเสถียร มากกว่า -80 dBm  เหตุผล: RSSI มีหน่วยเป็น dBm ซึ่งวัดในสเกลลอการิทึมและมีค่าเป็นลบ ตัวเลขที่เข้าใกล้ 0 มากกว่า (ติดลบน้อยกว่า) จะหมายถึงกำลังคลื่นวิทยุที่ภาครับได้รับสูงกว่า ทำให้สัญญาณมีความเข้มสูง รบกวนต่ำ และเสถียรกว่าค่าที่ติดลบมาก
...
ทราบรูปแบบความปลอดภัย: ทำให้ระบบทราบล่วงหน้าว่า AP เป้าหมายใช้โหมดความปลอดภัยแบบใด (เช่น WIFI_AUTH_OPEN, WIFI_AUTH_WPA2_PSK, หรือ WIFI_AUTH_WPA2_ENTERPRISE)  การตั้งค่าโครงสร้างการเชื่อมต่อ: ESP32 ต้องใช้ข้อมูล authmode ในการกำหนดสเปกและชุดโครงสร้างความปลอดภัย (Key Exchange / Handshake Protocol) ก่อนเริ่มกระบวนการ Authentication & Association หากตั้งค่าโหมดการเข้ารหัสไม่ตรงกับ AP การยืนยันตัวตนและการส่งรหัสผ่านจะไม่สำเร็